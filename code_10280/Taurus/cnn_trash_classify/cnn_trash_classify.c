...
#define MODEL_FILE_TRASH    "/userdata/models/cnn_trash_classify/fishdata_inst.wk" 
...
#define AUDIO_SCORE        70  
#define AUDIO_FRAME        30       // 每15帧识别一次，可自行配置

#define MULTIPLE_OF_EXPANSION 100   // Multiple of expansion
#define UNKOWN_WASTE          9    // Unkown Waste，这里更改了*******
...
HI_S32 CnnTrashClassifyLoadModel(uintptr_t* model, OsdSet* osds)
{
    ...
    *model = ret < 0 ? 0 : (uintptr_t)self;
    SAMPLE_PRT("load cnn fish classify model, ret:%d\n", ret);

    /** Uart串口初始化，这里加了串口*/
    uart1Fd = UartOpenInit();
    if (uart1Fd < 0) printf("uart1 open failed\r\n");
...
//改名称了
    offset += snprintf_s(buf + offset, size - offset, size - offset - 1, "fish classify: {");
    for (HI_U32 i = 0; i < itemNum; i++) {
        ...
        SAMPLE_PRT("----fish Classify result----num:%d, score:%d\n", item->num, score);
        switch (item->num) {
            case 0u:
                trashName = "hetun_0";
                UartSendRead(uart1Fd,hetun_0); 
                break;
            case 1u:
                trashName = "hudieyu_1";
                UartSendRead(uart1Fd,hudieyu_1);
                break;
            case 2u:
                trashName = "jinyu_2";
                UartSendRead(uart1Fd,jinyu_2);
                break;
            case 3u:
                trashName = "jiyu_3";
                UartSendRead(uart1Fd,jiyu_3);
                break;
            case 4u:
                trashName = "luohanyu_4";
                UartSendRead(uart1Fd, luohanyu_4);
                break;
            case 5u:
                trashName = "longyu_5";
                UartSendRead(uart1Fd, longyu_5);
                break;
            case 6u:
                trashName = "nianyu_6";
                UartSendRead(uart1Fd, nianyu_6);
                break;
            case 7u:
                trashName = "qingdaofu_7";
                UartSendRead(uart1Fd, qingdaofu_7);
                break;
            case 8u:
                trashName = "shiban_8";
                UartSendRead(uart1Fd, shiban_8);
                break;    
            default:
                trashName = "Unkown fish";
                //UartSendRead(uart1Fd, nofish); // 无效值
                break;
        }
...
{
    SAMPLE_PRT("begin CnnFishClassifyCal\n");
    ...
    SAMPLE_CHECK_EXPR_RET(ret < 0, ret, "CnnFishClassifyFlag cal FAIL, ret=%x\n", ret);
