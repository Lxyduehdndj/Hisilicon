...
void UartSendRead(int fd, refuseClassification refuseType)
{
    ...
    unsigned char writeBuffer2[4] = {0, 2, 0, 2};//从2开始
    unsigned char writeBuffer3[4] = {0, 2, 0, 3};
    unsigned char writeBuffer4[4] = {0, 2, 0, 4};
    unsigned char writeBuffer5[4] = {0, 2, 0, 5};
    unsigned char writeBuffer6[4] = {0, 2, 0, 6};
    unsigned char writeBuffer7[4] = {0, 2, 0, 7};
    unsigned char writeBuffer8[4] = {0, 2, 0, 8};
    unsigned char writeBuffer9[4] = {0, 2, 0, 9};
    unsigned char writeBuffer10[4] = {0, 2, 0, 10};
    unsigned char readBuff[16] = {0};
//LHH
#ifdef  EXPANSION_BOARD
//#ifdef ROBOT_BOARD
    switch (refuseType) {
        ...
        case jiyu_3:
            ...
            break;
        case luohanyu_4:
            HisignallingMsgSend(fd, writeBuffer6, sizeof(writeBuffer6)/sizeof(writeBuffer4[6]));
            printf("send data:%d\r\n",writeBuffer6[3]);
            break;
        case longyu_5:
        ...
        case qingdaofu_7:
            ...
            break;
        case shiban_8:
            HisignallingMsgSend(fd, writeBuffer10, sizeof(writeBuffer10)/sizeof(writeBuffer10[0]));
            printf("send data:%d\r\n",writeBuffer10[3]);
            break;
        default:
            //HisignallingMsgSend(fd, writeBuffer9, sizeof(writeBuffer9)/sizeof(writeBuffer9[0]));
            printf("send data:nofish\r\n");
            break;
    }
#endif
